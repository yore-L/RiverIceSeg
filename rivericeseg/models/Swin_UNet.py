...
class MockConfig:
    def __init__(self):
        # 数据配置
        self.DATA = type('', (), {'IMG_SIZE': 1024})()

        # 模型主配置
        self.MODEL = type('', (), {
            'DROP_RATE': 0.1,  # 模型层面的丢弃率
            'DROP_PATH_RATE': 0.2,  # 深度路径丢弃率
            'PRETRAIN_CKPT': None,  # 预训练权重路径
            'NUM_CLASSES': 2,  # 二分类任务
            'SWIN': type('', (), {  # SWIN子配置
                'PATCH_SIZE': 4,  # 适合1024的patch尺寸
                'IN_CHANS': 3,  # 输入通道数
                'EMBED_DIM': 96,  # 初始嵌入维度
                'DEPTHS': [2, 2, 18, 2],  # 调整深度处理大尺寸
                'NUM_HEADS': [3, 6, 12, 24],
                'WINDOW_SIZE': 8,
                'MLP_RATIO': 4.0,
                'QKV_BIAS': True,
...
def test_high_resolution():
    ...
    try:
        ...
        # 2. 验证配置完整性
        assert hasattr(config.MODEL, 'DROP_RATE'), "MODEL必须包含DROP_RATE属性"
        assert hasattr(config.MODEL.SWIN, 'DEPTHS'), "SWIN配置缺少DEPTHS"
        assert config.MODEL.SWIN.DEPTHS == [2, 2, 18, 2], "网络深度配置错误"
        ...
        # 5. 生成测试数据 (小批量避免显存溢出)
        test_input = torch.randn(1, 3, 1024, 1024).to(device)  # batch_size=1
        ...
        # 7. 验证输出形状
        expected_shape = (1, 2, 1024, 1024)  # (batch, classes, H, W)
        assert output.shape == expected_shape, (
            f"形状错误: 预期{expected_shape} 实际{output.shape}\n"
            "可能原因:\n"
            "1. 下采样次数过多导致特征图过小\n"
            "2. 最终上采样层配置错误\n"
            "3. Patch尺寸与输入尺寸不兼容"
        ...
        # assert output.min() >= 0 and output.max() <= 1, "输出值应在[0,1]范围内"
        ...
        print("\n!!! 测试失败 !!!")
        raise RuntimeError(f"配置或模型错误: {str(e)}") from e
