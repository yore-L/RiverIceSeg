...
class Encoder(nn.Module):
    ...
    def forward(self, x):  # torch.Size([1, 3, 256, 256])
        ...
        x = self.stage_1(x)  # torch.Size([1, 64, 256, 256])
        # pool_indice_1保留了第一个池化层的索引
        x, pool_indice_1 = nn.MaxPool2d(2, stride=2, return_indices=True)(x)  # torch.Size([1, 64, 128, 128])
...
class SegNet(nn.Module):
    ...
    def forward(self, x):  # torch.Size([1, 3, 256, 256])
        x, pool_indices = self.encoder(x)  # torch.Size([1, 512, 8, 8])

        # 池化索引上采样
        x = nn.MaxUnpool2d(2, 2, padding=0)(x, pool_indices[4])  # torch.Size([1, 512, 16, 16])
        x = self.upsample_1(x)  # torch.Size([1, 512, 16, 16])
...
input_tensor = torch.randn(1, 3, 1024, 1024)  # Batch size: 1, Channels: 3 (RGB), Height: 256, Width: 256

# 实例化模型
num_classes = 2  # 例如：PASCAL VOC数据集有21个类别
