...
# class SmartCropV1(T.transforms):
#     def __init__(self, crop_size=512, max_ratio=0.75, ignore_index=12, nopad=False):
#         super().__init__()
#         self.crop = T.RandomResizedCrop((crop_size, crop_size))
#         self.max_ratio = max_ratio
#         self.ignore_index = ignore_index
#
...
#             cropped_mask = self.crop(mask)
#             unique, counts = torch.unique(cropped_mask, return_counts=True)
#             counts = counts[unique != self.ignore_index]
#             if len(counts) > 1 and torch.max(counts) / torch.sum(counts) < self.max_ratio:
#                 break
#             if count > 10:
#                 break
#             count += 1
