...
CLASSES = ('background', 'river_ice')
PALETTE = [[0, 0, 0], [255, 255, 255]]  # 背景为白色，主体为蓝色
...
def train_aug(img, mask):
    ...
    crop_aug = Compose([RandomScale(scale_list=[0.75, 1.0, 1.25, 1.5], mode='value'),
                        SmartCropV2(crop_size=512, num_classes=2, class_interest=[0, 1], class_ratio=[0.1, 0.25], max_ratio=0.75),
...
class TrainDataset(Dataset):
    def __init__(self, data_root='data/Train', img_dir='images_png', mosaic_ratio=0.25,
                 mask_dir='masks_png_convert', img_suffix='.tif', mask_suffix='.tif',
    ...
    def get_img_ids(self, data_root, img_dir, mask_dir):
        urban_img_filename_list = os.listdir(osp.join(data_root, 'river_ice', img_dir))
        urban_mask_filename_list = os.listdir(osp.join(data_root, 'river_ice', mask_dir))
        assert len(urban_img_filename_list) == len(urban_mask_filename_list)
        urban_img_ids = [(str(id.split('.')[0]), 'river_ice') for id in urban_img_filename_list]

        rural_img_filename_list = os.listdir(osp.join(data_root, 'river_ice', img_dir))
        rural_mask_filename_list = os.listdir(osp.join(data_root, 'river_ice', mask_dir))
        assert len(rural_img_filename_list) == len(rural_mask_filename_list)
        rural_img_ids = [(str(id.split('.')[0]), 'river_ice') for id in rural_img_filename_list]
    ...
    def load_img_and_mask(self, index):
        ...
        # img = np.array([dataset.GetRasterBand(i).ReadAsArray() for i in range(5)])
...
val_dataset = TrainDataset(data_root='data/Val', mosaic_ratio=0.0, transform=val_aug)


class TestDataset(Dataset):
    def __init__(self, data_root='data/Test', img_dir='images_png/',
                 img_suffix='.tif',  mosaic_ratio=0.0,
